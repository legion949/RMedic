...
library(ggplot2)
library(gplots)
library(Hmisc)  # Esta tarda mucho en cargar...
library(plotly)
...
num_control <- function(BASE=NULL, cols=c(1:ncol(BASE))){
  ...
  nombres <- c("Celdas Vacías", "Vacío", "Letras Minúsculas", "Letras Mayúsculas", "Espacios", "Simbolos", "Comas", "No Punto", "Solo 1 Punto", "Dos o mas puntos", "Números", "OK")
  ORDEN <- list()
  FRASE <- list()
 ...
 # col_eleccion <- c(1,3:7,10)
      ...
      for( h in 1:cantidad) {
        ...
        if ( sum(d1)  == 0) {
          ...
          if (sum(d1,d2) == 0) {            
            ...
            if ( sum(d6) == 0)  DT[h,  6] <- TRUE   # Simbolos
            ...
            if (sum(d7, d6) == digitos)  DT[h,12] <- TRUE   # Todo OK
  ...
  #                 "Dos o mas puntos", "Números", "OK")
    ...
    FRASE[[n]][[12]] <- c("Las siguientes celdas presentan en formato correcto: solo números y pueden contener un punto como separador decimal.")
