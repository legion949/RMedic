...
output$calc1 <- renderUI({
  
  
  opciones <-  c("Probabilidad", "Valores Z", "Valores de la variable")
  radioButtons(inputId = "calc1", 
                     label = "A partir de...", 
                     choices = opciones,
...
output$calc2 <- renderUI({
  
  if (!is.null(input$calc1)) {
    
  opciones <- c("Probabilidad", "Valores Z", "Valores de la variable")
  dt <- opciones == input$calc1
  opciones[dt] <- NA
  opciones <- na.omit(opciones)
  
  checkboxGroupInput(inputId = "calc2", 
                     label = "Calcular...", 
                     choices = opciones,
                     selected = opciones)
  }
...
numericInput("prob1", "Ingrese un valor de probabilidad:", min=0,  max=1, step= 0.01, value=0.60)
})
...
output$prob2 <- renderUI({
  ...
  opciones <- c("Centrada (Simétrica)", 
                "De -Inf a un Valor Z",
                "De un valor Z a +Inf",
                "Otro caso...")
  
  radioButtons(inputId = "prob2", 
               label = "Detalle de probabilidad", 
               choices = opciones,
               selected = opciones[1])
})
...
#  par(mai=c(1,3,1,1), mar=(c(5,9,4,2)+0.1), mgp=c(4,1,0)) 
  ...
  plot(x_mod, y, xlab="Valores de Z", 
       ylab="Frecuencias Relativas", 
       main="Distribución Normal Estándard", 
       axes=F, col="white")
  ...
  # s <- seq(from= x1, to= x2, by= 0.001)
  # for(i in 1:length(s)){
  #   segments(x0= s[i], y0= 0, x1= s[i], y1= dnorm(s[i]), col= input$color)
  #   # curve(dnorm(x, mean, sd),xlim= c(Nmin, Nmax), xlab="Valores de Z", ylab="Frecuencia Relativa", main="Distribución Normal Estándard", add=T)
  #   
  # } # Fin for i
  # 
