...
Graficos2Q_01_RMedicHelp_UI <- function(id) {
  
  ns <- NS(id)
  
  fluidRow(
    column(4, 
           radioButtons(inputId = "help_graficos_2q",
    ...
    column(8,
           br(),
           conditionalPanel(condition = "input.help_graficos_2q == 1",
                            div(
                              h3("RMedic Here!"),
                              HTML(
           ...
           conditionalPanel(condition = "input.help_graficos_2q == 2",
                            div(
...
## Segmento del server
Graficos2Q_01_RMedicHelp_SERVER <- function(input, output, session,
                                            minibase,
                                            decimales,
                                            control_ejecucion,
                                            tablas_2q) {
