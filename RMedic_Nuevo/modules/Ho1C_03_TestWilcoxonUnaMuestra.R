...
Ho1C_03_TestWilcoxonUnaMuestra_UI <- function(id) {
...
## Segmento del server
Ho1C_03_TestWilcoxonUnaMuestra_SERVER <- function(input, output, session, 
                                                      minibase,
                                                      decimales,
                                                      control_ejecucion,
                                                      tablas_1c,
  ...
  The_Test <- reactive({
    ...
    Test_1C_TestWilcoxon_UnaMuestra( input_base = minibase(),
                              input_tipo_prueba = input$tipo_prueba_ho, 
                              input_mediana_ho = input$valor_bajo_ho,
  ...
  output$armado_ho <- renderUI({
    ...
    div(
      h2("Test de Wilcoxon (una muestra)"),
      "Nota: para la utilización del test Wilcoxon la variable debe ser numérica (ya sea cuantitativa u 
      ordinal representada con números).", 
      br(),
      br(),
      ...
      br(),
      h3("Tabla Resumen del Test de Wilcoxon (una muestra)"),
