...
Ho2C_05_TestTApareado_UI <- function(id) {
...
## Segmento del server
Ho2C_05_TestTApareado_SERVER <- function(input, output, session, 
                                    minibase, 
                                    decimales,
                                    control_ejecucion,
                                    alfa) {
  ...
  The_Test <- reactive({
   ...
   # if(is.null(input$valor_bajo_ho)) return(NULL)
    ...
    Test_2C_TestT_DosMuestras_Apareado( input_base = minibase(),
  ...
  output$armado_ho <- renderUI({
    ...
    div(
      h2("Test t (Dos muestras apareadas)"),
      "Nota: para la utilización del test t (Dos muestras apareadas) ambas 
     ...
     # uiOutput(ns("opciones_ho")),
      ...
      br(),
      h3("Tabla de Requisitos test t (Dos muestras apareadas)"),
      ...
      br(),
      h3("Tabla Resumen del test t (Dos muestras apareadas)"),
