...
Ho2C_07_TestHomogenedadDeVarianzasFisher_UI <- function(id) {
...
## Segmento del server
Ho2C_07_TestHomogenedadDeVarianzasFisher_SERVER <- function(input, output, session, 
                                    minibase, 
                                    decimales,
                                    control_ejecucion,
                                    alfa) {
 ...
 #   if(is.null(input$tipo_prueba_ho)) return(NULL)
  #  if(is.null(input$valor_bajo_ho)) return(NULL)
    ...
    Test_2C_TestHomogeneidadDeVarianzas_Fisher( input_base = minibase(),
