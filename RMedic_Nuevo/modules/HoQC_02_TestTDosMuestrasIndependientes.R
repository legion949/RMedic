...
HoQC_02_TestTDosMuestrasIndependientes_UI <- function(id) {
...
## Segmento del server
HoQC_02_TestTDosMuestrasIndependientes_SERVER <- function(input, output, session, 
                                         minibase, 
                                         decimales,
                                         control_ejecucion,
                                         alfa) {
  ...
  The_Test <- reactive({
    ...
    Test_QC_TestT_DosMuestras_Independientes( input_base = minibase(),
  ...
  output$armado_ho <- renderUI({
    ...
    div(
      h2("Test t (Dos muestras independientes)"),
      "Nota: para la utilización del test t (Dos muestras independientes) ambas 
      ...
      br(),
      h3("Tabla de Requisitos test t (Dos muestras independientes)"),
      ...
      br(),
      h3("Tabla Resumen del test t (Dos muestras independientes)"),
