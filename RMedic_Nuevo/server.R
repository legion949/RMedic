  ...
  output$OpcionesCIE_parte01 <- renderUI({
    ...
    if(!is.null(Base01()[[1]])) {
      ...
      selectInput(inputId = "cie_columna", 
                  ...
                  choices =  c("Seleccione una..." = "", armado),
                  selected = NA)  
    ...
    if (!is.null(Base01())) {
      ...
      if (!is.null(Base02()) && !is.null(input$cie_categoria)) {
        ...
        texto_completo02 <- c("
                            ...
                            <b>Unidades seleccionadas (Filas seleccionadas o repeticiones seleccionadas):</b> _nrowBase02_ unidades.<br/>
                              ")
  ...
  output$BASE_SALIDA <- renderDataTable({
    ...
    if (!is.null(Base01())) {
      ...
      if(RMedic_general()){
        ...
        mi_base <- Base01()[[1]]
        if(input$cie_especificaciones == 2) mi_base <- Base02()[[1]]
