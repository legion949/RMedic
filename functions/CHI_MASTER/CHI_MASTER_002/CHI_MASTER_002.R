...
TABLAS_DF <- DF02(MINI, input_decimales=input_decimales, input_marginal = FALSE)
TABLA <- TABLAS_DF[[1]][[1]]
...
# Analisis de la Consultora
{
  ...
  valor_p_interno <- round2(valor_p_interno, input_decimales)
  CONSULTORA$valor_p_interno <- valor_p_interno
  
  # Valor p externo
  if (valor_p_interno < 0.001) valor_p_externo <- c("<<0.001") else  valor_p_externo <- valor_p_interno
...
RESUMEN[1,3] <- metodo
RESUMEN[1,4] <- round2(ORIGINAL$Test_Chi$statistic, input_decimales)
if (metodo == "Clásico") RESUMEN[1,5] <- round2(ORIGINAL$Test_Chi$parameter, input_decimales) else RESUMEN[1,5] <- "No Corresponde"
...
###
  ...
  valor_referencia <- round2(valor_referencia, input_decimales)
  ...
  for(n in 1:ncol(TENDENCIA)) for(k in 1:nrow(TENDENCIA)){
    ...
    if (valor_p_interno >= input_alfa) TENDENCIA[k,n] <- "Sin Tendencia"
    }
  ...
  # Agregado...
  # Si no se rechaza Ho...
  # Muestra en todo "Sin Tendencia
  if (input_alfa > valor_p_interno) {
  for(n in 1:ncol(TENDENCIA)) for(k in 1:nrow(TENDENCIA)){
    
    
    TENDENCIA[k,n] <- "Sin Tendencia" 
  }
  }  # Fin if
...
if (1 == 2) {
 ...
 JA$ANALISIS$ORIGINAL$Test_Chi$expected
 
 JA$ANALISIS$ORIGINAL$Test_Chi$residuals
 
 JA$ANALISIS$ORIGINAL$Test_Chi$stdres
