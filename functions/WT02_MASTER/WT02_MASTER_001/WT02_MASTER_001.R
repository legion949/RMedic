...
WT02_MASTER <- function(input_datos=NULL, input_decimales=2, input_alfa=0.05){
  ...
  # En esta funcion WT02 es para el test de muestras apareadas...
  ...
  # La funcion WT02... no va a forzar a tomar como factor a la 2da columna...
  ...
  WT02$DETALLES <- list()
  WT02$ANALISIS <- list()
  WT02$REQUERIMIENTOS <- list()
  WT02$SALIDA_ARMADA <- list()  
  
  # Input
  {
  WT02$input$datos <- input_datos
  WT02$input$decimales <- input_decimales
  WT02$input$alfa <- input_alfa
  }# Fin Input
  ################################################################
  ...
  # Detalles
  {
    ...
    WT02$DETALLES$nombre_X1 <- colnames(MINI)[1]
    WT02$DETALLES$nombre_X2 <- colnames(MINI)[2]
  ...
  # # # CONTROLES
  {
    ...
    WT02$CONTROLES <- control_OK
    
    WT02$CONTROL_GENERAL <- control_general
  ...
  if (control_general == TRUE) {
    ...
    WT02$REQUERIMIENTOS$HOMOGENEIDAD <- list()
    WT02$REQUERIMIENTOS$NORMALIDAD <- list()
    ...
    WT02$REQUERIMIENTOS$HOMOGENEIDAD <- HOMOGENEIDAD
    WT02$REQUERIMIENTOS$NORMALIDAD <-  NORMALIDAD
    
    
    
    WT02$REQUERIMIENTOS$RESUMEN_NH <- RESUMEN_NH  
    ...
    frase3_v1 <- c("Otra opción, en principio posible para este pool de datos, sería aplicar un test t para muestras apareadas.", "<br/>",
                   "Pero, esa idea queda descartada al no se cumple con el requisito distribución normal de la diferencia entre X2 y X1.", "<br/>",
                   "Para analizar este pool de datos y poder sacar conclusiones correctas, debe utilizarse el test de test de Wilcoxon para muestras apareadas.")
    
    frase3_v2 <- c("Otra opción, en principio posible para este pool de datos, sería aplicar un test t para muestras apareadas.", "<br/>",
                   "Pero, esa idea queda descartada al no se cumple con el requisito de homogeneidad de varianzas de ambos grupos.", "<br/>",
                   "Para analizar este pool de datos y poder sacar conclusiones correctas, debe utilizarse el test de Wilcoxon para muestras apareadas.")
    
    
    frase3_v3 <- c("Otra opción, en principio posible para este pool de datos, sería aplicar un test t para muestras apareadas.", "<br/>",
                   "Pero, esa idea queda descartada al no se cumple con los requisitos de normalidad de la diferencia de las variables
                    y la homogeneidad de varianzas entre los grupos.","<br/>",
                   "Para analizar este pool de datos y poder sacar conclusiones correctas, debe utilizarse el test de Wilcoxon para muestras apareadas.")
    ...
    frase3_v4 <- c("Se cumplen los requisitos necesarios de normaldiad y de homogeneidad de varianzas de los grupos." , "<br/>",
                   "En vez de utilizarse el test de Wilcoxon para muestras apareadas, podría optarse por utilizar un test t para muestras apareadas.", "<br/>",
                   "Ambos test serán válidas para sacar conclusiones sobre este pool de datos.")
    ...
    WT02$REQUERIMIENTOS$FRASE_NH <- FRASE_NH
    ...
    #  ANALISIS$ORIGINAL <-  t.test(VR ~ FACTOR, paired=FALSE, var.equal=arg_var.equal, conf.level= (1-input_alfa))
      ...
      X1 <- MINI[,1]
      X2 <- MINI[,2]
      ANALISIS$ORIGINAL <-   wilcox.test(X1, X2, paired = TRUE, alternative = "two.sided", conf.level=(1-input_alfa),conf.int = TRUE)
      ...
      nombres <- c("X1", "X2", "Test Estadístico", "Valor 'V'", "GL", "Valor p", "Decisión") 
      ...
      RESUMEN_TT[1,3] <- "Test de Wilcoxon para muestras apareadas"
      RESUMEN_TT[1,4] <- round2(ANALISIS$ORIGINAL$statistic, input_decimales)
      RESUMEN_TT[1,5] <- "No corresponde"
    ...
    WT02$ANALISIS$CONSULTORA$frase2_test_t_html <- frase_elegida
    ...
    SALIDA_ARMADA$FRASE_INICIAL <- "Test de Wilcoxon para muestras apareadas"
    
    SALIDA_ARMADA$RESUMEN_TT <- WT02$ANALISIS$CONSULTORA$RESUMEN
    
    SALIDA_ARMADA$FRASE_TT <-    WT02$ANALISIS$CONSULTORA$frase2_test_t_html
    
    SALIDA_ARMADA$RESUMEN_NH <- WT02$REQUERIMIENTOS$RESUMEN_NH
    
    SALIDA_ARMADA$FRASE_NH <- WT02$REQUERIMIENTOS$FRASE_NH
  ...
  WT02$SALIDA_ARMADA <- SALIDA_ARMADA
